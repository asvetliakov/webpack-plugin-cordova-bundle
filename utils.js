...
module.exports = async function processPluginXML(pluginXmlPath, platform) {
  ...
  if (!parsed.plugin) {
    throw new Error(
      `Plugin xml: ${pluginXmlPath} does not define <plugin> xml tag`
    );
  }
  let resultFiles = [];
  ...
  if (parsed.plugin.platform) {
    const platforms = Array.isArray(parsed.plugin.platform)
      ? parsed.plugin.platform
      : [parsed.plugin.platform];
    const thisPlatform = platforms.find(
      p => p["$"] && p["$"].name === platform
    );
  ...
  resultFiles = resultFiles.filter(
    (v, i, arr) => !arr.find((n, ni) => n.name === v.name && ni > i)
  );
  return resultFiles;
};
